# Nexus Pascal
`Nexus Pascal` 是一个用于开发FreePascal程序的 VSCode 应用扩展.
...
- 默认会自动搜索FPC路径，如果找不到，请设置FPC的路径为系统搜索路径，或者在插件里设置 `nexusPascal.env.PP = FPC程序全路径`.
...
#### 使用方法
1. 在 Pascal Projects 视图中点击 "+" 按钮或使用 `Ctrl+Shift+P` → "Nexus Pascal: Create New Project"
2. 从可用模板中选择
3. 项目文件创建时变量自动替换
4. 构建任务已配置并可直接使用
